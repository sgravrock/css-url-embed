...
function embedUrl(grunt, url, urlContentInBuffer, mimeType, fileContent) {
...
function resolveMimeTypeAndEmbedUrl(grunt, url, urlContent, fileContent) {
	var urlContentInBuffer = Buffer.from(urlContent);
	embedUrl(grunt, url, urlContentInBuffer, mime.getType(url), fileContent);
}

function processUrl(grunt, fileContent, url, options, baseDir) {
	...
	try {
		...
		resolveMimeTypeAndEmbedUrl(grunt, url, urlContent, fileContent);
	} catch (e) {
		throw new Error('Failed to embed "' + url + '"', {cause: e});
	}
}

function processFile(grunt, fileSrc, fileDest, options) {
	...
	for (const url of uniqueEmbeddableUrls) {
		processUrl(grunt, fileContentRef, url, options, baseDir);
	}
