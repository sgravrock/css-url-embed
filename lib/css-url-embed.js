...
function embedUrlAndGoToNext(grunt, url, urlContentInBuffer, mimeType, fileContent, nextUrl) {
...
function resolveMimeTypeEmbedUrlAndGoToNext(grunt, url, urlContent, fileContent, nextUrl, options) {
  ...
  if (mmmagicMimeType && options.useMimeTypeSniffing) {
    mmmagicMimeType.detect(urlContentInBuffer, function(error, mimeType) {
      ...
      embedUrlAndGoToNext(grunt, url, urlContentInBuffer, mimeType, fileContent, nextUrl);
    });
  } else {
    embedUrlAndGoToNext(grunt, url, urlContentInBuffer, mime.getType(url), fileContent, nextUrl);
  }
}

function processNextUrl(grunt, fileContent, currentUrlIndex, urlArray, options, baseDir, isVerbose, finishCallback) {
  if (++currentUrlIndex === urlArray.length) {
    finishCallback();
  } else {
    processUrl(grunt, fileContent, currentUrlIndex, urlArray, options, baseDir, isVerbose, finishCallback);
  }
}

function processUrl(grunt, fileContent, currentUrlIndex, urlArray, options, baseDir, isVerbose, finishCallback) {
  var url = urlArray[currentUrlIndex];
  var nextUrl = processNextUrl.bind(null, grunt, fileContent, currentUrlIndex, urlArray, options, baseDir, isVerbose, finishCallback);
  ...
  try {
    ...
    if (REMOTE_URL_REGEX.test(url)) {
      request({ url: url, encoding: null }, function(error, response, body) {
        ...
        resolveMimeTypeEmbedUrlAndGoToNext(grunt, url, body, fileContent, nextUrl, options);
      });
    } else {
      ...
      resolveMimeTypeEmbedUrlAndGoToNext(grunt, url, urlContent, fileContent, nextUrl, options);
    }
  } catch (e) {
    grunt.log.error(e);
...
function processFile(grunt, fileSrc, fileDest, options, callback) {
  try {
    ...
    processUrl(grunt, fileContentRef, 0, uniqueEmbeddableUrls, options, baseDir, isVerbose, function() {
