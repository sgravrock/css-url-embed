...
async function processFile(fileSrc, fileDest) {
	...
	for (const url of uniqueEmbeddableUrls) {
		processUrl(fileContentRef, url, baseDir, mime);
	}
...
function processUrl(fileContent, url, baseDir, mime) {
	...
	try {
		...
		const mimeType = mime.getType(urlFullPath);
